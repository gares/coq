...
let loc_before = function
  | { Tok.blanks_before = []; loc; _ } -> { loc with Loc.ep = loc.Loc.bp }
  | { Tok.blanks_before = { Tok.Blank.loc; _ } :: _; _ } ->
       { loc with Loc.ep = loc.Loc.bp }
let pos_of { Tok.pos; _ } = pos
