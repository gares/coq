...
The {\tt par:} goal selector is equivalent to {\tt all:} but dispatches the
goals to a pool of Coq worker processes.  The number of worker processes can be
controlled via the {\tt -async-proofs-tac-j} command line flag.  The default
value is {\texttt 2}.  Only ground goals, i.e. not containing existential
variables, can be handled by the {\texttt par} goal selector.
