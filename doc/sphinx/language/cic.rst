...
inductive definition on sort :math:`\Set` and generate constraints
between universes for inductive definitions in the Type hierarchy.
...
.. _Empty-and-singleton-eliminaton-for-SProp:

Inductive types in :math:`\SProp` have even stricter restrictions on
their eliminations than those in :math:`\Prop`: to eliminate into any
relevant type (which includes types in :math:`\Prop`) if the inductive
has a constructor it must have no arguments.
