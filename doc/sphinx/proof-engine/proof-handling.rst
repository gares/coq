...
- In CoqIDE, position the cursor on or just after the tactic whose proof term you
  want to compare to the current proof term, then select `View / Show Proof with Diffs`
  from the menu or press `ALT-S`.  Differences will be shown applying the current `Show Diffs`
  setting from the `View` menu.
