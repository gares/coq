...
module SentenceId : sig
  ...
  type sentence = private {
    ...
    edit_id : int;
    mutable state_id : Stategraph.state_id option;
  }

  val mk_sentence :
    start:GText.mark -> stop:GText.mark -> flag list -> sentence

  val assign_state_id : sentence -> Stategraph.state_id -> unit
  ...
  let assign_state_id s id =
    ...
    assert(id <> Stategraph.dummy_state_id);
    s.state_id <- Some id
...
object(self)
  ...
  val mutable initial_state = Stategraph.initial_state_id
  ...
  method show_goals =
    ...
    | Interface.Fail x -> self#handle_failure x
  ...
  method raw_coq_query phrase =
    ...
    let next = function
    | Interface.Fail (_, _, err) -> display_error err; Coq.return () (* XXX*)
    | Interface.Good (_, msg) ->
  ...
  method private mark_as_needed sentence =
    ...
    let tags =
      (if List.mem `PROCESSING sentence.flags then to_process else
       ...
       processed)
      ::
      (if List.mem `UNSAFE sentence.flags then [unjustified] else [])
    in
  ...
  method private is_dummy_id id =
    match id with
    | Interface.Edit 0 -> true
    | Interface.State id when id = Stategraph.dummy_state_id -> true
    | _ -> false

  method private process_feedback msg =
    let id = msg.Interface.id in
    if self#is_dummy_id id || Searchstack.is_empty cmd_stack then () else
    let sentence =
      let finder () s =
        match s.state_id, id with
        | Some id', Interface.State id when id = id' -> `Stop s
        | _, Interface.Edit id when id = s.edit_id -> `Stop s
        | _ -> `Cont () in
    ...
    | _ -> if sentence <> None then Minilib.log "Unsupported feedback message"
  ...
  method private process_interp_error queue sentence loc msg id =
    ...
    messages#clear;
    messages#push Interface.Error msg;
    assert((Searchstack.top cmd_stack).state_id = Some id); (* XXX: comments *)
    self#show_goals)
  ...
  method join_document =
   let next = function
     | Interface.Good _ -> Coq.return ()
     | Interface.Fail _ -> Coq.return () (* FIXME *) in
  ...
  method private segment_to_be_cleared until =
    ...
    with Not_found ->
      ...
      None, Searchstack.to_list cmd_stack
...
(*         buffer#remove_tag Tags.Script.tooltip ~start ~stop; *)
    ...
    let rec undo until =
      ...
      | Interface.Good () -> cleanup n seg; conclusion ()
      | Interface.Fail (safe_id, loc, msg) ->
          if loc <> None then messages#push Interface.Error "Fixme LOC";
          messages#push Interface.Error msg;
  ...
  method handle_failure (safe_id, (loc : (int * int) option), msg) =
    ...
    messages#clear;
    messages#push Interface.Error msg;
    self#backtrack_until (fun _ id _ _ -> id = Some safe_id)
  ...
  method tactic_wizard l =
    ...
    let try_phrase phrase stop more =
      ...
      let next = function
      | Interface.Fail (_, l, str) -> (* FIXME: check *)
        display_error (l, str);
...
(*         messages#add msg; *)
  ...
  method initialize =
    ...
    in
    Coq.seq add_load_path Coq.PrintOpt.enforce
