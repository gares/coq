...
let coqtopInterpretToPoint ~id params : 'a ExecutionManager.actions Lwt.t =
  ...
  DocumentManager.interpret_to_position ~progress_hook document loc >>= fun (new_doc, proof, actions) ->
  Hashtbl.replace documents uri new_doc;
  send_highlights uri new_doc >>= fun () ->
...
let coqtopStepBackward ~id params : 'a ExecutionManager.actions Lwt.t =
  ...
  let uri = params |> member "uri" |> to_string in
  let document = Hashtbl.find documents uri in
  DocumentManager.interpret_to_previous document >>= fun (new_doc, proof, actions) ->
  Hashtbl.replace documents uri new_doc;
  send_highlights uri new_doc >>= fun () ->
  publish_diagnostics uri new_doc >>= fun () ->
  match proof with
...
let coqtopStepForward ~id params : 'a ExecutionManager.actions Lwt.t =
  ...
  let uri = params |> member "uri" |> to_string in
  let document = Hashtbl.find documents uri in
  DocumentManager.interpret_to_next document >>= fun (new_doc, proof, actions) ->
  Hashtbl.replace documents uri new_doc;
  send_highlights uri new_doc >>= fun () ->
  publish_diagnostics uri new_doc >>= fun () ->
  match proof with
...
let coqtopInterpretToEnd ~id params : 'a ExecutionManager.actions Lwt.t =
  ...
  let document = Hashtbl.find documents uri in
  let progress_hook = progress_hook uri in
  DocumentManager.interpret_to_end ~progress_hook document >>= fun (new_doc, proof, actions) ->
  Hashtbl.replace documents uri new_doc;
  send_highlights uri new_doc >>= fun () ->
  publish_diagnostics uri new_doc >>= fun () ->
  match proof with
