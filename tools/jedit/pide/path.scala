...
Aceess to the real environment, subclass to alter "expand"
Symbol ~ is now associated with HOME (and not USER_HOME)
Symbol ~~ is still hardcoded to be ISABELLE_HOME
    ...
    def eval(elem: Path.Elem): List[Path.Elem] =
      elem match {
        case Path.Variable(s) =>
          Path.explode(Standard_System.getenv_strict(s)).elems
        case x => List(x)
      }
