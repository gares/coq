...
object Pretty
{
  ...
  object Block
  {
    def apply(i: Int, body: XML.Body): XML.Tree =
      XML.Elem(Markup(Slave_Markup.BLOCK, Slave_Markup.Indent(i)), body)

    def unapply(tree: XML.Tree): Option[(Int, XML.Body)] =
      tree match {
        case XML.Elem(Markup(Slave_Markup.BLOCK, Slave_Markup.Indent(i)), body) =>
          Some((i, body))
        case _ => None
      }
  }
  ...
  object Break
  {
    def apply(w: Int): XML.Tree =
      XML.Elem(Markup(Slave_Markup.BREAK, Slave_Markup.Width(w)),
        List(XML.Text(Symbol.spaces(w))))

    def unapply(tree: XML.Tree): Option[Int] =
      tree match {
        case XML.Elem(Markup(Slave_Markup.BREAK, Slave_Markup.Width(w)), _) => Some(w)
        case _ => None
      }
  }
