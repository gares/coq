...
final class Outer_Syntax private(
  ...
  control: Set[String] = Set.empty, /* TODO: make this extensible? */
  val completion: Completion = Completion.empty)
{
  ...
  def + (name: String, kind: String, replace: String): Outer_Syntax =
    new Outer_Syntax(
      ...
      lexicon + name,
      control,
      if (control(kind)) completion else completion + (name, replace))

  def + (name: String, kind: String): Outer_Syntax = this + (name, kind, name)
  def + (name: String): Outer_Syntax = this + (name, Outer_Syntax.MINOR)
  ...
  def is_command(name: String): Boolean =
    keyword_kind(name) match {
      case Some(kind) => kind != Outer_Syntax.MINOR
      case None => false
    }
