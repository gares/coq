...
object Protocol
{
  ...
  val command_status_markup: Set[String] =
    Set(Slave_Markup.ACCEPTED, Slave_Markup.FINISHED, Slave_Markup.FAILED,
      Slave_Markup.FORKED, Slave_Markup.JOINED)

  def command_status(status: Status, markup: Markup): Status =
    markup match {
      case Markup(Slave_Markup.ACCEPTED, _) => status.copy(accepted = true)
      case Markup(Slave_Markup.FINISHED, _) => status.copy(finished = true)
      case Markup(Slave_Markup.FAILED, _) => status.copy(failed = true)
      case Markup(Slave_Markup.FORKED, _) => status.copy(forks = status.forks + 1)
      case Markup(Slave_Markup.JOINED, _) => status.copy(forks = status.forks - 1)
      case _ => status
    }
  ...
  def clean_message(body: XML.Body): XML.Body =
    body filter { case XML.Elem(Markup(Slave_Markup.NO_REPORT, _), _) => false case _ => true } map
      { case XML.Elem(markup, ts) => XML.Elem(markup, clean_message(ts)) case t => t }

  def message_reports(msg: XML.Tree): List[XML.Elem] =
    msg match {
      case elem @ XML.Elem(Markup(Slave_Markup.REPORT, _), _) => List(elem)
 ...
 def is_tracing(msg: XML.Tree): Boolean =
    msg match {
      case XML.Elem(Markup(Slave_Markup.TRACING, _), _) => true
      case _ => false
    }

  def is_warning(msg: XML.Tree): Boolean =
    msg match {
      case XML.Elem(Markup(Slave_Markup.WARNING, _), _) => true
      case _ => false
    }

  def is_error(msg: XML.Tree): Boolean =
    msg match {
      case XML.Elem(Markup(Slave_Markup.ERROR, _), _) => true
      case _ => false
    }
/*
  def is_state(msg: XML.Tree): Boolean =
    msg match {
      case XML.Elem(Markup(Slave_Markup.WRITELN, _),
...
*/
  ...
  private val include_pos =
    Set(/*Isabelle_Markup.BINDING, Isabelle_Markup.ENTITY,*/ Slave_Markup.REPORT,
      Slave_Markup.POSITION)
...
trait Protocol extends Slave_Process
{
  /* commands */
      ...
      def encode_edit(name: Document.Node.Name)
        ...
        variant(List(
          ...
          { case Document.Node.Header(Exn.Res(deps)) =>
              val dir = Standard_System.posix_path(name.dir)
              val imports = deps.imports.map(_.node)
              // FIXME val uses = deps.uses.map(p => (Standard_System.posix_path(p._1), p._2))
              val uses = deps.uses
              (Nil,
