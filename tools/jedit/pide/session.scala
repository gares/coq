              ...
              thy_load: Thy_Load)
{
  ...
  val syslog_messages = new Event_Bus[Slave_Process.Output]
  val raw_output_messages = new Event_Bus[Slave_Process.Output]
  val all_messages = new Event_Bus[Slave_Process.Message]  // potential bottle-neck
        ...
        case Text_Edits(previous, text_edits, version_result) =>
          ...
          val (doc_edits, version) =
            Timing.timeit("Thy_Syntax.text_edits", timing) {
              Prover.syntax.text_edits(prover_syntax, prev, text_edits)
            }
  ...
  private case class Messages(msgs: List[Slave_Process.Message])
  ...
  private val (_, session_actor) = Simple_Thread.actor("session_actor", daemon = true)
  {
    ...
    object receiver
    {
      private var buffer = new mutable.ListBuffer[Slave_Process.Message]

      def flush(): Unit = synchronized {
        if (!buffer.isEmpty) {
          val msgs = buffer.toList
          this_actor ! Messages(msgs)
          buffer = new mutable.ListBuffer[Slave_Process.Message]
        }
      }
      def invoke(msg: Slave_Process.Message): Unit = synchronized {
        buffer += msg
        msg match {
          case output: Slave_Process.Output =>
            if (output.is_syslog)
              syslog >> (queue =>
                {
    ...
    var prover: Option[Slave_Process with Protocol] = None
    ...
    def handle_output(output: Slave_Process.Output)
    //{{{
    {
      def bad_output(output: Slave_Process.Output)
      {
        if (verbose)
      ...
      output.properties match {
        ...
        case Slave_Markup.Assign_Execs if output.is_protocol =>
        ...
        case Slave_Markup.Removed_Versions if output.is_protocol =>
          XML.content(output.body).mkString match {
        ...
        case Slave_Markup.Invoke_Scala(name, id) if output.is_protocol =>
          Future.fork {
        ...
        case Slave_Markup.Cancel_Scala(id) if output.is_protocol =>
          System.err.println("cancel_scala " + id)  // FIXME actually cancel JVM task

        case Slave_Markup.Ready if output.is_protocol =>
            phase = Session.Ready

        case Slave_Markup.Loaded_Theory(name) if output.is_protocol =>
          thy_load.register_thy(name)

        case Slave_Markup.Command_Decl(name, kind) if output.is_protocol =>
          prover_syntax += (name, kind)

        case Slave_Markup.Keyword_Decl(name) if output.is_protocol =>
          prover_syntax += name
      ...
      receiveWithin(delay_commands_changed.flush_timeout) {
        ...
        case Start(timeout, args) if prover.isEmpty =>
          if (phase == Session.Inactive || phase == Session.Failed) {
            ...
            prover = Some(new Slave_Process(timeout, receiver.invoke _, 
              use_socket, program, env, args) with Protocol)
          }
        ...
        case Messages(msgs) =>
          msgs foreach {
            case input: Slave_Process.Input =>
