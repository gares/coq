/*  Title:      slave_process.scala
    Author:     Makarius, Enrico
    Options:    :folding=explicit:collapseFolds=1:

Slave process management -- always reactive due to multi-threaded I/O.
*/
...
object Slave_Process
{
  ...
  class Input(name: String, args: List[String]) extends Message
  {
    override def toString: String =
      XML.Elem(Markup(Slave_Markup.PROVER_COMMAND, List((Markup.NAME, name))),
        args.map(s =>
          List(XML.Text("\n"),
            XML.elem(Slave_Markup.PROVER_ARG, 
              YXML.parse_body(s)))).flatten).toString
  }
  ...
  class Output(val message: XML.Elem) extends Message
  {
    ...
    def is_init = kind == Slave_Markup.INIT
    def is_exit = kind == Slave_Markup.EXIT
    def is_stdout = kind == Slave_Markup.STDOUT
    def is_stderr = kind == Slave_Markup.STDERR
    def is_system = kind == Slave_Markup.SYSTEM
    def is_status = kind == Slave_Markup.STATUS
    def is_report = kind == Slave_Markup.REPORT
    def is_protocol = kind == Slave_Markup.PROTOCOL
...
class Slave_Process(
    ...
    receiver: Slave_Process.Message => Unit = Console.println(_),
    use_socket : Boolean,
    program: String,
    env: Map[String,String] = null,
    args: List[String] = Nil)
{
  import Slave_Process._

  /* output */

  private def system_output(text: String)
  {
    receiver(new Output(XML.Elem(
      Markup(Slave_Markup.SYSTEM, Nil), List(XML.Text(text)))))
  }
  ...
  {
    if (kind == Slave_Markup.INIT) system_channel.accepted()
    if (kind == Slave_Markup.PROTOCOL)
  ...
  private val process =
    try {
      val cmdline = program :: system_channel.slave_args ::: args
      new Standard_System.Managed_Process(false, env, cmdline: _*)
    }
  ...
  private def terminate_process()
  {
    try { process.terminate }
      ...
      case e: IOException =>
        system_output("Failed to terminate "+program+": " + e.getMessage) 
    }
  }
  ...
  private val process_manager = Simple_Thread.fork("process_manager")
  {
    ...
    if (startup_failed) {
      output_message(Slave_Markup.EXIT, "Return code: 127: ")
      process.stdin.close
      Thread.sleep(300)
      terminate_process()
      process_result.join
      ...
      val to_join = List(standard_input, stdout, stderr, command_input, message)
      for ((thread, _) <- to_join) thread.join
      system_output("process_manager terminated")
      output_message(Slave_Markup.EXIT, "Return code: " + rc.toString)
    }
  ...
  def terminate() {
    close()
    system_output("Terminating slave process")
    terminate_process()
  }
        ...
        while (!finished) {
          ...
          receive {
            ...
            case Close =>
              ...
              finished = true
            case bad =>
              System.err.println(name + ": ignoring bad message " + bad)
          }
          //}}}
        }
      }
  ...
  private def physical_output_actor(err: Boolean): (Thread, Actor) =
  {
    val (name, reader, markup) =
      if (err) ("standard_error", process.stderr, Slave_Markup.STDERR)
      else ("standard_output", process.stdout, Slave_Markup.STDOUT)
  ...
  private def message_actor(stream: InputStream): (Thread, Actor) =
  {
    ...
    Simple_Thread.actor(name) {
      ...
      def read_chunk(decode: Boolean): XML.Body =
      {
        ...
        if (decode)
          YXML.parse_body_failsafe(
            Standard_System.decode_chars(Symbol.decode, buf, 0, n))
        else
          List(XML.Text(
            Standard_System.decode_chars(s => s, buf, 0, n).toString))
        //}}}
      }
      ...
      try {
          ...
          try {
            ...
            header match {
              case List(XML.Elem(Markup(name, props), Nil))
                  if name.size == 1 && 
                     Slave_Markup.inflate.isDefinedAt(name(0)) =>
                val kind = Slave_Markup.inflate(name(0))
                val body = read_chunk(kind != Slave_Markup.PROTOCOL)
        ...
        case e: IOException =>
          system_output("Cannot read message:\n" + e.getMessage)
        case e: Protocol_Error =>
          system_output("Malformed message:\n" + e.getMessage)
      }
      stream.close
  ...
  def input_bytes(name: String, args: Array[Byte]*): Unit =
    command_input._2 ! Input_Chunks(
                         Standard_System.string_bytes(name) :: args.toList)
