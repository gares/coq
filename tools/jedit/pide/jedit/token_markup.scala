...
object Token_Markup
{
        ...
        handler: TokenHandler, raw_line: Segment): TokenMarker.LineContext =
    {
      ...
      val context1 =
      {
        val (styled_tokens, context1) =
          if (line_ctxt.isDefined && Prover.session.is_ready) {
            val syntax = Prover.session.recent_syntax()
            val (tokens, ctxt1) = syntax.scan_context(line, line_ctxt.get)
            val styled_tokens =
              tokens.map(tok => (JEdit_Prover.rendering.token_markup(syntax, tok), tok))
  ...
  {
    if (buffer.getMode.getName == "isabelle") {
