...
class Session_Dockable(view: View, position: String) extends Dockable(view: View, position: String)
{
  ...
  private val syslog = new TextArea(Prover.session.current_syslog())
  ...
  private val tabs = new TabbedPane {
    ...
    selection.index =
    {
      val index = JEdit_Prover.Int_Property("session-panel.selection", 0)
      if (index >= pages.length) 0 else index
    }
    listenTo(selection)
    reactions += {
      case SelectionChanged(_) =>
        JEdit_Prover.Int_Property("session-panel.selection") = selection.index
    }
  }
  ...
  val session_phase = new Label(Prover.session.phase.toString)
  session_phase.border = new SoftBevelBorder(BevelBorder.LOWERED)
  session_phase.tooltip = "Prover status"

  private val cancel = new Button("Cancel") {
    reactions += { case ButtonClicked(_) => Prover.cancel_execution() }
  }
  cancel.tooltip = jEdit.getProperty("isabelle.cancel-execution.label")

  private val check = new Button("Check") {
    reactions += { case ButtonClicked(_) => JEdit_Prover.check_buffer(view.getBuffer) }
  }
  check.tooltip = jEdit.getProperty("isabelle.check-buffer.label")

  private val logic = JEdit_Prover.logic_selector(JEdit_Prover.Property("logic"))
  logic.listenTo(logic.selection)
  logic.reactions += {
    case SelectionChanged(_) => JEdit_Prover.Property("logic") = logic.selection.item.name
  }
    ...
    override def paintComponent(gfx: Graphics2D)
    {
      nodes_status.get(node_name) match {
        case Some(st) if st.total > 0 =>
          ...
          for {
            (n, color) <- List(
              (st.unprocessed, JEdit_Prover.rendering.unprocessed1_color),
              (st.running, JEdit_Prover.rendering.running_color),
              (st.warned, JEdit_Prover.rendering.warning_color),
              (st.failed, JEdit_Prover.rendering.error_color)) }
          {
            gfx.setColor(color)
  ...
  {
    Swing_Thread.now {
      val snapshot = Prover.session.snapshot()
      ...
      val nodes_status1 =
        (nodes_status /: iterator)({ case (status, (name, node)) =>
            if (Prover.thy_load.is_loaded(name.theory)) status
    ...
    loop {
      react {
        case output: Slave_Process.Output =>
          if (output.is_syslog)
            Swing_Thread.later {
              val text = Prover.session.current_syslog()
  ...
  override def init() {
    Prover.session.syslog_messages += main_actor
    Prover.session.phase_changed += main_actor
    Prover.session.commands_changed += main_actor
  }
