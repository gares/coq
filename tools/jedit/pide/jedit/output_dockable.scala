...
class Output_Dockable(view: View, position: String) extends Dockable(view, position)
{
  ...
  private val html_panel =
    new HTML_Panel(JEdit_Prover.font_family(), scala.math.round(JEdit_Prover.font_size()))
  {
    override val handler: PartialFunction[HTML_Panel.Event, Unit] =
    { case HTML_Panel.Mouse_Click(elem, event) if false => /* Sendback!!!
  ...
  {
    Swing_Thread.now {
      html_panel.resize(JEdit_Prover.font_family(),
        scala.math.round(JEdit_Prover.font_size() * zoom_factor / 100))
    }
  }
            ...
            case Some(cmd) if !restriction.isDefined || restriction.get.contains(cmd) =>
              ...
              val filtered_results =
                snapshot.state.command_state(snapshot.version, cmd).results.iterator
                  .map(_._2).filter(
                  { // FIXME not scalable
                    case XML.Elem(Markup(Slave_Markup.TRACING, _), _) => show_tracing
                    case _ => true
                  }).toList
  ...
  override def init()
  {
    Prover.session.global_settings += main_actor
    Prover.session.commands_changed += main_actor
    Prover.session.caret_focus += main_actor
  }
  ...
  private val delay_resize =
    Swing_Thread.delay_first(Prover.session.update_delay) { handle_resize() }
  ...
  private val zoom = new JEdit_Library.Zoom_Box(factor => { zoom_factor = factor; handle_resize() })
