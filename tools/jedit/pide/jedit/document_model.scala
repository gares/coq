...
class Document_Model(val session: Session, val buffer: Buffer, val name: Document.Node.Name)
{
  ...
  def node_header(): Document.Node_Header = Exn.Exn(ERROR("No theory header in Coq"))
  ...
  def perspective(): Text.Perspective =
  {
    ...
    Text.Perspective(
      for {
        doc_view <- JEdit_Prover.document_views(buffer)
  ...
  private object pending_edits  // owned by Swing thread
  {
    ...
    {
      flush()
      session.init_node(name, node_header(), perspective(), JEdit_Prover.buffer_text(buffer))
    }
