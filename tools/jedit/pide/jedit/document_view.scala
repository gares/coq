...
class Document_View(val model: Document_Model, val text_area: JEditTextArea)
{
  ...
  {
    Swing_Thread.now {
      html_panel.resize(JEdit_Prover.font_family(), scala.math.round(JEdit_Prover.font_size()))
    }
  }
...
*/
  ...
  private val mouse_motion_listener = new MouseMotionAdapter {
    override def mouseMoved(e: MouseEvent) {
      ...
      if (!model.buffer.isLoaded) exit_control()
        ...
        JEdit_Prover.buffer_lock(model.buffer) {
          ...
          _highlight_range =
            if (control) {
              val offset = text_area.xyToOffset(x, y)
              JEdit_Prover.rendering.subexp(snapshot, Text.Range(offset, offset + 1))
            }
            else None
          for (Text.Info(range, _) <- _highlight_range) invalidate_range(range)
        }
  ...
  private val tooltip_painter = new TextAreaExtension
  {
    override def getToolTipText(x: Int, y: Int): String =
    {
      robust_body(null: String) {
        ...
        val tip =
          if (control) JEdit_Prover.rendering.tooltip(snapshot, range)
          else JEdit_Prover.rendering.tooltip_message(snapshot, range)
        tip.map(JEdit_Prover.tooltip(_)) getOrElse null
      }
    }
  }
          ...
          JEdit_Prover.buffer_lock(model.buffer) {
            ...
            for (i <- 0 until physical_lines.length) {
              if (physical_lines(i) != -1) {
                ...
                // gutter icons
                JEdit_Prover.rendering.gutter_message(snapshot, line_range) match {
                  case Some(icon) =>
  ...
  {
    val delay_repaint =
      Swing_Thread.delay_first(Prover.session.update_delay) { repaint() }
  }
