    ...
    if (snapshot.is_outdated) None
      ...
      val results =
        snapshot.cumulate_markup[(Protocol.Status, Int)](
          ...
          Some(Protocol.command_status_markup + Slave_Markup.WARNING + Slave_Markup.ERROR),
          {
            case ((status, pri), Text.Info(_, XML.Elem(markup, _))) =>
              if (markup.name == Slave_Markup.WARNING) (status, pri max warning_pri)
              else if (markup.name == Slave_Markup.ERROR) (status, pri max error_pri)
  ...
  def tooltip_message(snapshot: Document.Snapshot, range: Text.Range): Option[String] =
  {
    val msgs =
      snapshot.cumulate_markup[SortedMap[Long, String]](range, SortedMap.empty,
        Some(Set(Slave_Markup.WRITELN, Slave_Markup.WARNING, Slave_Markup.ERROR)),
        {
          case (msgs, Text.Info(_, msg @ XML.Elem(Markup(markup, Slave_Markup.Serial(serial)), _)))
          if markup == Slave_Markup.WRITELN ||
              markup == Slave_Markup.WARNING ||
              markup == Slave_Markup.ERROR =>
            msgs + (serial ->
  ...
  def gutter_message(snapshot: Document.Snapshot, range: Text.Range): Option[Icon] =
  {
    val results =
      snapshot.cumulate_markup[Int](range, 0,
        Some(Set(Slave_Markup.WARNING, Slave_Markup.ERROR)),
        {
          case (pri, Text.Info(_, XML.Elem(Markup(Slave_Markup.WARNING, _), body))) =>
            body match {
              case List(XML.Elem(Markup(Slave_Markup.LEGACY, _), _)) => pri max legacy_pri
              case _ => pri max warning_pri
            }
          case (pri, Text.Info(_, XML.Elem(Markup(Slave_Markup.ERROR, _), _))) =>
            pri max error_pri
        })
      ...
      snapshot.cumulate_markup[Int](range, 0,
        Some(Set(Slave_Markup.WRITELN, Slave_Markup.WARNING, Slave_Markup.ERROR)),
        {
          case (pri, Text.Info(_, XML.Elem(Markup(name, _), _))) =>
            name match {
              case Slave_Markup.WRITELN => pri max writeln_pri
              case Slave_Markup.WARNING => pri max warning_pri
              case Slave_Markup.ERROR => pri max error_pri
              case _ => pri
            }
        })
    for {
      ...
      for {
        Text.Info(r, result) <-
          snapshot.cumulate_markup[(Option[Protocol.Status], Option[Color])](
            ...
            Some(Protocol.command_status_markup + Slave_Markup.BAD + Isabelle_Markup.HILITE),
            {
              case (((Some(status), color), Text.Info(_, XML.Elem(markup, _))))
              if (Protocol.command_status_markup(markup.name)) =>
                (Some(Protocol.command_status(status, markup)), color)
              case (_, Text.Info(_, XML.Elem(Markup(Slave_Markup.BAD, _), _))) =>
                (None, Some(bad_color))
