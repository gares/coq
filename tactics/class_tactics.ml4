...
let pf_filtered_hyps gls = 
  Goal.V82.hyps gls.Evd.sigma (fst (sig_it gls))
...
let compare (pri, _, _, res) (pri', _, _, res') =
  let nbgoals s =
    List.length (fst (sig_it s)) + nb_empty_evars (sig_sig s)
  in
  let pri = pri - pri' in
	 ...
	 | _ -> 
	     second.skft
		  ...
		  in aux s' (gls'::acc) fk'' gls)
	       fk {it = (gl,info); sigma = s; eff = Declareops.no_side_effects })
...
let make_autogoals ?(only_classes=true) ?(st=full_transparent_state) hints gs evm' =
  ...
  { it = List.map_i (fun i g -> 
    let (gl, auto) = make_autogoal ~only_classes ~st cut (Some (fst g)) {it = snd g; sigma = evm'; eff = Declareops.no_side_effects } in
      (gl, { auto with auto_depth = [i]})) 1 gs; sigma = evm'; eff = Declareops.no_side_effects }
...
let resolve_one_typeclass env ?(sigma=Evd.empty) gl =
  ...
  let gls = { it = gl ; sigma = sigma; eff= Declareops.no_side_effects } in (* XXX eff *)
