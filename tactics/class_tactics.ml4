	 ...
	 | _ -> 
	     second.skft
		  ...
		  in aux s' (gls'::acc) fk'' gls)
	       fk {it = (gl,info); sigma = s;eff=[]}) (* XXX eff??? *)
...
let make_autogoals ?(only_classes=true) ?(st=full_transparent_state) hints gs evm' =
  ...
  { it = list_map_i (fun i g -> 
    let (gl, auto) = make_autogoal ~only_classes ~st cut (Some (fst g)) {it = snd g; sigma = evm';eff=[]} in (* XXX eff??? *)
      (gl, { auto with auto_depth = [i]})) 1 gs; sigma = evm'; eff = []}
...
let resolve_one_typeclass env ?(sigma=Evd.empty) gl =
  ...
  let gls = { it = gl ; sigma = sigma; eff=[] } in (* XXX eff *)
