...
let default_pr_subgoal n sigma =
  let rec prrec p = function
	...
	if Int.equal p 1 then
          let pg = default_pr_goal { sigma=sigma ; it=g; eff=Declareops.no_side_effects } in
...
let default_pr_subgoals ?(pr_first=true) close_cmd sigma seeds stack goals =
  ...
  let print_multiple_goals g l =
    if pr_first then
      default_pr_goal { it = g ; sigma = sigma; eff=Declareops.no_side_effects } ++
      pr_rec 2 l
    else 
      pr_rec 1 (g::l)
  in
  ...
  | [g],[] when not !Flags.print_emacs ->
      let pg = default_pr_goal { it = g ; sigma = sigma; eff=Declareops.no_side_effects } in
      v 0 (
...
let pr_goal_by_id id =
  ...
  in
  try
    Proof.in_proof p (fun sigma -> pr {it=g;sigma=sigma;eff=Declareops.no_side_effects})
