...
and execute ~hack ~progress_hook st task : (state * [> `Workers of action ] Lwt.t list) Lwt.t =
  let open Lwt.Infix in
...
let observe ~hack progress_hook schedule id st : (state * [> `Workers of action ] Lwt.t list) Lwt.t =
  let open Lwt.Infix in
