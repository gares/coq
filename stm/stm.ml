...
let stm_vernac_interp ?route id st { verbose; expr } : Vernacstate.t =
  ...
  *)
  ...
  if is_filtered_command cmd then
    ...
    match CLwt.main_run (Vernacinterp.interp ?verbosely:(Some verbose) ~st expr) with
    | CLwt.Ok (st,_) -> st
    | CLwt.Error iexn -> Exninfo.iraise  iexn
  end
 ...
 *)
  ...
  let () =
    ...
    Declare.Proof.map_proof_lwt (fun p ->
