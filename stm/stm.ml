...
type cmd_t = ast * Id.t list * int option
...
module State : sig
  ...
  let is_cached ?(cache=`No) id =
    if Stateid.equal id !cur_id then begin
      ...
      | { state = None } when cache = `Yes -> freeze `No id
      | { state = None } when cache = `Shallow -> freeze `Shallow id 
      | _ -> ());
      true
    end else
      match VCS.get_info id with
...
module Slaves : sig
  ...
  let set_perspective idl =
    ...
    TaskQueue.reorder_tasks (fun task1 task2 ->
      let { Task.t_start = a1; t_stop = b1 } = task1 in
      let { Task.t_start = a2; t_stop = b2 } = task2 in
  ...
  and reach ?(redefine_qed=false) ?(cache=cache) id =
    ...
    let step, cache_step =
      ...
      | `Cmd (x,_,Some n) -> (fun () ->
            ...
            Partac.vernac_interp n view.next id x
          ), cache
      | `Fork (x,_,_,_) -> (fun () ->
            reach view.next; vernac_interp id x;
