...
and shift_problem ((current,t),_,na) sigma pb =
  ...
  let pb =
    { pb with
       env = snd (push_rel sigma (LocalDef (make_annot na r,current,ty)) env);
       tomatch = tomatch;
	  ...
	  match alias with
		  ...
		  try
		    ...
		    let neq = eq_id avoid id in
                      sigma, LocalDef (make_annot (Name neq) Sorts.Relevant, mkRel 0, eq_t) :: sign, 2, Id.Set.add neq avoid
