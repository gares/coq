...
let interp_ast vernac_st ast =
    ...
    Vernacinterp.interp ~st:vernac_st ast >>= fun result -> (* TODO set status to "Delegated" *)
...
let prepare_task ~hack ~progress_hook st task : state * prepared_task =
  ...
  | OpaqueProof (id,ids) ->
     ...
     let (st,remote_mapping), jobs = CList.fold_left_map (remotize ~hack) (st,remote_mapping) ids in
...
let observe ~hack progress_hook schedule id st : (state * events) Lwt.t =
  ...
  let rec build_tasks id tasks =
    ...
    | _ ->
      ...
      let (base_id, task) = task_for_sentence schedule id in
  ...
  let st, tasks = CList.fold_left_map (prepare_task ~hack ~progress_hook) st tasks in
