 ...
 let make_one_eq cur =
  ...
  let rettyp_l = quick_chop nparrec (deconstruct_type rettyp) in
    ...
    *)
	...
	match kind_of_term c with
        | Rel x -> mkRel (x-nlist+ndx), Declareops.no_side_effects
        | Var x -> mkVar (id_of_string ("eq_"^(string_of_id x))), Declareops.no_side_effects
        | Cast (x,_,_) -> aux (applist (x,a))
        | App _ -> assert false
        | Ind (kn',i as ind') -> 
            if eq_mind kn kn' then mkRel(eqA-nlist-i+nb_ind-1), Declareops.no_side_effects
            else begin
              try
                let eq, eff =
  ...
  (* do the [| C1 ... =>  match Y with ... end
    ...
    let ar = Array.make n ff in
        let eff = ref Declareops.no_side_effects in
	for i=0 to n-1 do
...
let make_eq_decidability mind =
  ...
  [|Pfedit.build_by_tactic (Global.env())
    ...
    (compute_dec_tact ind lnamesparrec nparrec)|], Declareops.no_side_effects
