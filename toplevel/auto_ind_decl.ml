  ...
  let rettyp_l = quick_chop nparrec (deconstruct_type rettyp) in
    ...
    *)
	...
	match kind_of_term c with
        ...
        | App _ -> assert false
        | Ind (kn',i as ind') -> 
            if eq_mind kn kn' then mkRel(eqA-nlist-i+nb_ind-1), []
            else begin
              try
		let a = Array.of_list a in
                let c, eff = find_scheme (!beq_scheme_kind_aux()) (kn',i) in
                let eq = mkConst c in
                let eqa, effs = List.split (List.map aux (Array.to_list a)) in
                let eff = List.flatten (List.rev effs) @ eff in
                let eqa = Array.of_list eqa in
                let args =
                  Array.append (Array.map (fun x -> lift lifti x) a) eqa in
                if args = [||] then eq, eff
...
let eqI ind l =
  ...
  in (if eA = [||] then e else mkApp(e,eA)), eff
  ...
  let lbI, eff'' =
    try let c, eff = find_scheme lb_scheme_kind ind in mkConst c, eff with
            Not_found -> error (
              ...
              " equality is required.")
  in
  tclTHENSEQ [Tactics.emit_side_effects (eff'' @ eff' @ eff);
