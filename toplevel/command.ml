...
let interp_definition bl red_option fail_evar c ctypopt =
  ...
  let imps,ce =
	...
	{ const_entry_body = Future.from_val (body,Declareops.no_side_effects);
...
let do_definition ident k bl red_option c ctypopt hook =
  let (ce, evd, imps as def) = interp_definition bl red_option (not (Flags.is_program_mode ())) c ctypopt in
    if Flags.is_program_mode () then
      ...
      assert(sideff = Declareops.no_side_effects);
...
let build_wellfounded (recname,n,bl,arityc,body) r measure notation =
  ...
  let hook, recname, typ = 
	...
	let ce =
          { const_entry_body = Future.from_val (Evarutil.nf_evar !isevars body,Declareops.no_side_effects);
            const_entry_secctx = None;
...
let declare_fixpoint local ((fixnames,fixdefs,fixtypes),fiximps) indexes ntns =
  if List.mem None fixdefs then
    ...
    let fixdecls = List.map (fun c -> c, Declareops.no_side_effects) fixdecls in
