...
let classify_vernac e =
  let rec static_classifier = function
    ...
    | VernacList _ -> anomaly "VernacList not handled by Vernac"
    | VernacLoad _ -> anomaly "VernacLoad not handled by Vernac"
    (* Nested vernac exprs *)
    | VernacProgram e -> static_classifier e
    | VernacLocality (_,e) -> static_classifier e
...
module StateCache : sig
  ...
  val define_state :
      ?cache:bool -> ('a -> unit) -> 'a -> state_id -> (exn -> exn) -> unit
  ...
  let install id =
    ...
    let s = 
      ...
      with Not_found -> anomaly "unfreezing a non existing state" in
  ...
  let define_state ?(cache=false) f x id g =
    if is_defined id then anomaly "defining a state twice";
    try f x; if cache then freeze id else cur_id := id (* optimization *)
    with e -> cur_id := dummy_state_id; raise (g e)
...
let string_of_transaction = function
  | Cmd t | Fork (t, _) -> 
      (try Pp.string_of_ppcmds (pr_vernac t) with _ -> "ERR")
  | Sideff (Some t) -> 
...
let reach_state ?cache f x id =
  let on_failure = function
    ...
    | Anomaly _ as e -> e
    | e -> graph := Dag.reach id false !graph; ErrorReachingState ([],id,e) in
  StateCache.define_state ?cache f x id on_failure;
  graph := Dag.reach id true !graph
     
let reach_known_state (perform : perform) ~cache id =
    ...
    let step, cache_step =
      ...
      | [ p1,Qed (x,(hd,_)); p2, Noop ] | [ p2, Noop ; p1,Qed (x,(hd,_)) ] ->
           ...
           | `NotOptimizable `Immediate -> assert (p1 = p2);
               (fun () -> reach p2; perform x; Proof_global.discard_all ())
           | `NotOptimizable _ ->
               (fun () ->
                 reach p2;
...
let get_head head = 
  ...
  with Not_found -> anomaly ("head " ^ head ^ " not found")
...
let find_proof_at_depth pl heads =
  list_try_find (function
    | h, (_, Proof (m, n), _) as x when n = pl -> x
    | _ -> failwith "find_proof_at_depth")
    heads
...
let process_transaction (f : perform) x =
  ...
  prerr_endline ("{{{ execute: "^Pp.string_of_ppcmds (pr_vernac x));
  ...
  try
    let rc = 
      ...
      | _, VtBack id -> 
        ...
        set_heads (Searchstack.top graph_history).hds;
        let dust = ref [] in
        let olddag = (Searchstack.top graph_history).dag in
        ...
        checkout_shallowest_proof_branch ();
        reach_known_state f ~cache:!Flags.ide_slave_mode id
      | _, VtStartProof mode ->
(*     XXX MOVE TO VCS
      begin
...
*)    
    ...
    rc
  with
  | (ErrorReachingState _ | Anomaly _) as e -> vcs_restore vcs; raise e
  | e -> anomaly ("execute: "^ Printexc.to_string e ^ "}}}")
...
let get_current_state () = (Searchstack.top graph_history).id
...
let get_all_proof_names () =
  List.map unmangle (list_map_filter proofname (heads()))
...
let _ = register_handler begin function
  | ErrorReachingState (_,id, e) ->
      Pp.v  0 
      (Pp.(++) (Pp.str"Reaching state ") 
        (Pp.(++) (Pp.str (string_of_state_id id)) 
          (Pp.(++) (Pp.cut()) (print e))))
  | _ -> raise Unhandled
end
