	...
	let (sp_projs,subst) =
	  ...
	  | Name fid ->
		...
		mkCase (ci, p, mkRel 1, [|branch|]) in
		  ...
		  let cie = {
                    const_entry_body =
                      Future.from_val (proj,Declareops.no_side_effects);
...
let declare_class finite def infer id idbuild paramimpls params arity fieldimpls fields
  ...
  let impl, projs =
	...
	let class_entry =
          { const_entry_body =
              Future.from_val (class_body,Declareops.no_side_effects);
            const_entry_secctx = None;
	...
	let proj_entry =
          { const_entry_body =
              Future.from_val (proj_body,Declareops.no_side_effects);
            const_entry_secctx = None;
