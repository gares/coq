...
module Refinable = struct
  ...
  let with_type t typ env rdefs _ _ =
    ...
    in
    rdefs := new_defs;
    j'.Environ.uj_val, Declareops.no_side_effects
  ...
  let constr_of_raw handle check_type resolve_classes rawc env rdefs gl info =
    ...
    in
      ...
      open_constr, Declareops.no_side_effects
  ...
  let constr_of_open_constr handle check_type (evars, c) env rdefs gl info =
    let delta = update_handle handle !rdefs evars in
      ...
      else c, Declareops.no_side_effects
...
let refine step env rdefs gl info =
  ...
  in
  { subgoals = subgoals }, Declareops.no_side_effects
...
let clear ids env rdefs gl info =
  ...
  { subgoals = [cleared_goal] }, Declareops.no_side_effects
...
let clear_body idents env rdefs gl info =
  ...
  { subgoals = [new_goal] }, Declareops.no_side_effects
...
let here goal value _ _ goal' _ =
  if equal goal goal' then
    value, Declareops.no_side_effects
  else
    raise UndefinedHere
...
module V82 = struct
  ...
  let dummy_goal =
    ...
    { Evd.it = build evk ; Evd.sigma = sigma; eff = Declareops.no_side_effects }
  ...
  let new_goal_with sigma gl extra_hyps =
    ...
    { Evd.it = build evk ; sigma = new_sigma; eff = Declareops.no_side_effects }
