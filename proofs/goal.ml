...
let interp_constr cexpr env rdefs _ _ =
  ...
  rdefs := defs ;
  c, Declarations.no_side_effects
...
module Refinable = struct
  ...
  let mkEvar handle env typ _ rdefs _ _ =
    ...
    handle := e::!handle;
    ev, Declarations.no_side_effects
  ...
  let with_type t typ env rdefs _ _ =
    ...
    in
    rdefs := new_defs;
    j'.Environ.uj_val, Declarations.no_side_effects
  ...
  let constr_of_raw handle check_type resolve_classes rawc env rdefs gl info =
    ...
    in
      ...
      open_constr, Declarations.no_side_effects
  ...
  let constr_of_open_constr handle check_type (evars, c) env rdefs gl info =
    let delta = update_handle handle !rdefs evars in
      ...
      else c, Declarations.no_side_effects
...
let refine step env rdefs gl info =
  ...
  in
  { subgoals = subgoals }, Declarations.no_side_effects
...
let clear ids env rdefs gl info =
  ...
  { subgoals = [cleared_goal] }, Declarations.no_side_effects
...
let clear_body idents env rdefs gl info =
  ...
  { subgoals = [new_goal] }, Declarations.no_side_effects
...
let rename_hyp id1 id2 env rdefs gl info =
  ...
  let new_concl = Term.replace_vars [id1,mkVar id2] old_concl in
...
module V82 = struct
  ...
  let dummy_goal =
    ...
    { Evd.it = build evk ; Evd.sigma = sigma; eff = [] }
  ...
  (* XXX should we change this API to take sideeffects into account? *)
  let new_goal_with sigma gl new_hyps =
    ...
    { Evd.it = build evk ; sigma = new_sigma; eff = [] }
