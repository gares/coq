...
type proofview = {
     ...
     comb : Goal.goal list;
     side_effects : Declarations.side_effects
     }
...
let emit_side_effects eff x = { x with side_effects = eff @ x.side_effects }
...
let list_of_sensitive s k env =
  ...
  try 
    ...
    Inner.set { step with solution = defs;
      side_effects = step.side_effects @ List.flatten effs } >>
    Inner.return tacs
...
let sensitive_on_proofview s env step =
  ...
  { step with
     ...
     side_effects = List.flatten side_effects @ step.side_effects }
...
module V82 = struct
  ...
  let tactic tac env =
    ...
    try
      let tac evd gl = 
        let glsigma  = tac { Evd.it = gl ; sigma = evd; eff = [] }  in
        let sigma = glsigma.Evd.sigma in
        let g = glsigma.Evd.it in
        ( g, sigma, glsigma.Evd.eff )
      in
    (* Old style tactics expect the goals normalized with respect to evars. *)
      let no_side_eff f x y = let a, b = f x y in a, b, [] in
      ...
      Inner.set { ps with solution=evd ; comb=sgs; 
        side_effects = ps.side_effects @ List.flatten (eff1 @ eff2) }
    with e when catchable_exception e ->
      tclZERO e env
