...
- [ ] Cc `@gares` to ensure that a `vsrocq-language-server` opam package is available in [`extra-dev`](https://github.com/rocq-prover/opam/tree/master/extra-dev) or [`released`](https://github.com/rocq-prover/opam/tree/master/released), respectively.
- [ ] Cc `@erikmd` and `@Justme0606` to ensure that the necessary configuration is ready to release the Docker images in [`rocq/rocq-prover`](https://hub.docker.com/r/rocq/rocq-prover) (gathering `rocq-prover` and `coq-bignums` opam packages).
