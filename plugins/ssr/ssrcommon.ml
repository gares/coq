...
let tacSIGMA = Goal.enter_one begin fun g ->
  let k = Goal.goal (Goal.assume g) in
...
let unsafe_intro env store decl b =
  ...
  Tactics.New.refine ~typecheck:true begin fun sigma ->
...
let rec decompose_assum env sigma t =
  let open Context in
  let t as goal = Reductionops.whd_allnolet env sigma t in
  ...
  | App(hd,args) when EConstr.isLetIn sigma hd -> (* hack *)
      ...
      decompose_assum env sigma (EConstr.mkApp (EConstr.Vars.subst1 v b, args))
  | _ -> CErrors.user_err
...
let tclINTRO ~id ~conclusion:k = Goal.enter begin fun gl ->
  ...
  let decl, t = decompose_assum env sigma g in
