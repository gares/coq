...
module State : sig
  ...
  val isPRINT : 'a Proofview.Goal.t -> Pp.t
...
let ns_print env sigma = function
  | None -> Pp.str "-"
  | Some t -> Printer.pr_econstr_env env sigma t
    ...
    | Some (IPatId ipat) ->
       ...
       let rec gen_eq_tac () = Goal.enter begin fun g ->
         ...
         then Ssrcommon.tclINTRO_ANON <*> gen_eq_tac ()
           ...
           Tactics.apply_type new_concl [case;erefl]
       end in
       gen_eq_tac () <*>
       intro_lhs <*>
       Ssrcommon.tclINTRO_ID ipat
...
let pushmoveeqtac cl c = Goal.enter begin fun g ->
  ...
  Unsafe.tclEVARS sigma <*>
  Tactics.apply_type (EConstr.mkProd (x, t, cl2)) [c; eqc]
end
