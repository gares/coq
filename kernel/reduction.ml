  ...
  match (fterm_of hd1, fterm_of hd2) with
    ...
    | (FProj (p1,c1), FProj (p2, c2)) ->
	 ...
	 form *)
      ...
      | Some (def1,s1) -> 
	eqappr env cv_pb l2r infos (lft1, whd def1 (s1 :: v1)) appr2 cuniv
      | None ->
	match unfold_projection infos p2 c2 with
	| Some (def2,s2) ->
	  eqappr env cv_pb l2r infos appr1 (lft2, whd def2 (s2 :: v2)) cuniv
	| None -> 
    ...
    | (FProj (p1,c1), t2) ->
	 ...
	 (match t2 with 
	  | FFlex fl2 ->
	     (match unfold_reference infos fl2 with
              | Some def2 ->
		 eqappr env cv_pb l2r infos appr1 (lft2, whd def2 v2) cuniv
              | None -> raise NotConvertible)
	  | _ -> raise NotConvertible))
    ...
    | (t1, FProj (p2,c2)) ->
	 ...
	 (match t1 with 
	  | FFlex fl1 ->
	     (match unfold_reference infos fl1 with
              | Some def1 ->
		 eqappr env cv_pb l2r infos (lft1, whd def1 v1) appr2 cuniv
              | None -> raise NotConvertible)
	  | _ -> raise NotConvertible))
    ...
    | (FInd (ind1,u1), FInd (ind2,u2)) ->
      if eq_ind ind1 ind2
      then
        (* first try equating instances *)
        let cuniv =
          try convert_instances false u1 u2 cuniv with
            exn ->
            ...
            if mind.Declarations.mind_polymorphic then
              ...
              if not (num_param_arity = CClosure.stack_args_size v1 && num_param_arity = CClosure.stack_args_size v2) then
                ...
                if cv_pb = CONV then compare_leq_inductives ~flex:false uinfind u2 u1 cuniv else cuniv
            else raise exn
        in
        convert_stacks env l2r infos lft1 lft2 v1 v2 cuniv
      else raise NotConvertible
