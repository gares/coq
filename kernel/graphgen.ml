...
(* FIXME treatment of litterals / min / var_min / max *)
module Make (Input : GraphIn) : GraphS with module Input := Input = struct
  ...
  let is_min_arc u = Level.is_min u.univ
  let is_max_arc u = Level.is_max u.univ
  let is_var_min_arc u = Level.is_var_min u.univ

  let get_min_arc g = repr g Level.min
  ...
  let check_universes_invariants g =
    ...
    UMap.iter (fun l u ->
        ...
        | Canonical u ->
           ...
           incr n_nodes
        | Equiv _ -> assert (not (Level.is_litteral l)))
              g.entries;
  ...
  let check_smaller g strict u v =
    ...
    if strict then
      ...
      is_min_arc arcu || is_max_arc arcv
      || (is_var_min_arc arcu && not (is_min_arc arcv))
  ...
  let empty =
    let add_lit_arc univ ilvl entries =
      UMap.add univ (Canonical {
               ...
               entries
    in
    (* FIXME: assumes that var_min <> min iff max exists*)
    match Level.max with
    | None ->
       let entries = add_lit_arc Level.var_min (-1) (add_lit_arc Level.min 0 UMap.empty) in
       let empty = { entries; index = (-2); n_nodes = 2; n_edges = 0 } in
       enforce_univ_lt Level.min Level.var_min empty
    | Some max ->
      let entries =
        UMap.empty |>
        add_lit_arc max (-2) |>
        add_lit_arc Level.var_min (-1) |>
        add_lit_arc Level.min 0
      in
      let empty = { entries; index = (-3); n_nodes = 3; n_edges = 0 } in
      empty |>
      enforce_univ_lt Level.min Level.var_min |>
      enforce_univ_lt Level.var_min max

  (* Prop = Set / HSet = HInf are forbidden here. *)
  let initial = empty

  let is_initial g = UMap.equal (==) g.entries initial.entries
  ...
  (* FIXME I have no clue what this does in Trunc mode *)
  let sort_universes g =
    ...
    let lowest_levels =
      UMap.mapi (fun u _ -> if Level.is_litteral u then 0 else 2)
                (UMap.filter
    ...
    let types = Array.init (max_lvl + 1) (function
                                          | 0 -> Level.min
                                          | 1 -> Level.var_min
                                          | n -> Level.of_path mp (n-2))
    in
    let g = Array.fold_left (fun g u ->
                let g, u = safe_repr g u in
                change_node g { u with rank = big_rank }) g types
    in
    let g = if max_lvl >= 2 then enforce_univ_lt Level.var_min types.(2) g else g in
    let g =
