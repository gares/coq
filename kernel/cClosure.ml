  ...
  match v.term with
    ...
    | FCaseT (ci,p,c,ve,env) ->
      if is_subs_id env && is_lift_id lfts then
        mkCase (ci, p, None, to_constr lfts c, ve)
      else
        let subs = comp_subs lfts env in
        mkCase (ci, subst_constr subs p, None,
            to_constr lfts c,
            Array.map (fun b -> subst_constr subs b) ve)
    | FCaseInvert (ci, p, (u,params,is), c, ve, env) ->
      let is = mkApp (mkIndU (ci.ci_ind, u),
                      Array.map (to_constr lfts) (Array.append params is))
      in
      if is_subs_id env && is_lift_id lfts then
        mkCase (ci, p, Some is, to_constr lfts c, ve)
      else
        let subs = comp_subs lfts env in
        mkCase (ci, subst_constr subs p, Some is,
            to_constr lfts c,
            Array.map (fun b -> subst_constr subs b) ve)
...
let strip_update_shift_app_red head stk =
  let rec strip_rec rstk h depth = function
    ...
    | ((ZcaseT _ | Zproj _ | Zfix _) :: _ | []) as stk -> (depth,List.rev rstk, stk) in
...
let strip_args_shift h stk =
  assert (match Mark.red_state h.mark with Red -> false | Norm | Cstr | Whnf -> true);
  let rec strip_rec h args = function
    ...
    | Zupdate m :: stk ->
      strip_rec (update ~share:true m h.mark h.term) args stk
...
let rec get_args n tys f e = function
    ...
    | ((ZcaseT _ | Zproj _ | Zfix _) :: _ | []) as stk -> (Inr {mark=mark Cstr Unknown;term=FLambda(n,tys,f,e)}, stk)
...
and knht info e t stk =
  match kind t with
    ...
    | Case(ci,p,Some is,t,br) ->
      let ind, args = decompose_appvect is in
      let args = Array.map (mk_clos e) args in
      let params, is = Array.chop ci.ci_npar args in
      let inv = match [@ocaml.warning "-4"] kind ind with
        | Ind (_,u) -> u, params, is
        | _ -> assert false
      in
      let term = FCaseInvert (ci, p, inv, mk_clos e t, br, e) in
...
let rec knr info tab m stk =
  ...
  | FEvar(ev,env) ->
      (match info.i_cache.i_sigma ev with
        ...
        | None -> (m,stk))
  | FCaseInvert (ci,_p,is,_c,v,env) when red_set info.i_flags fMATCH ->
    begin match case_inversion info tab ci is v with
